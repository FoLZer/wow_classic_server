...
#[test]
fn test_concurrent_reads_use_independent_cursors() {
    let temp = TempDir::new().unwrap();
    let archive = create_test_archive(
        temp.path(),
        "terrain.MPQ",
        &[
            ("World/Maps/Test/Test_1_1.adt", b"first tile"),
            ("World/Maps/Test/Test_1_2.adt", b"second tile"),
        ],
    );
    let mut chain = PatchChain::new();
    chain.add_archive(archive, 0).unwrap();
        ...
        for index in 0..16 {
            ...
            scope.spawn(move || {
                let (path, expected) = if index % 2 == 0 {
                    ("World/Maps/Test/Test_1_1.adt", b"first tile".as_slice())
                } else {
                    ("World/Maps/Test/Test_1_2.adt", b"second tile".as_slice())
                };
                for _ in 0..32 {
                    assert_eq!(chain.read_file_concurrent(path).unwrap(), expected);
                }
            });
        }
    });
