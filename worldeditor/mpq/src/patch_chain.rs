...
impl PatchChain {
    ...
    /// Read a file without mutably borrowing the patch chain.
    ///
    /// Each contributing archive uses an independent file cursor, allowing
    /// multiple reads from this patch chain to run concurrently.
    pub fn read_file_concurrent(&self, filename: &str) -> Result<Vec<u8>> {
        let lookup_key = crate::path::normalize_mpq_path(filename).to_uppercase();
        let Some(&archive_idx) = self.file_map.get(&lookup_key) else {
        ...
        if file_info.is_patch_file() {
            self.read_patched_file_concurrent(filename)
        } else {
            self.archives[archive_idx]
                .archive
                .read_file_concurrent(filename)
        }
    }

    /// Read a file asynchronously on the executor polling this future.
    ///
    /// Spawn this on an I/O or compute task pool so decompression and file I/O
    /// do not block an application's main thread.
    pub async fn read_file_async(&self, filename: &str) -> Result<Vec<u8>> {
        self.read_file_concurrent(filename)
    }
    ...
    fn read_patched_file_concurrent(&self, filename: &str) -> Result<Vec<u8>> {
        ...
        for entry in &self.archives {
            if let Ok(Some(file_info)) = entry.archive.find_file(filename) {
                if file_info.is_patch_file() {
                    let mut archive = entry.archive.read_handle()?;
                    if let Ok(patch_data) = archive.read_patch_file_raw(filename)
                        && let Ok(patch) = PatchFile::parse(&patch_data)
                    {
                        patches.push(patch);
                    }
                } else if base_data.is_none() {
                    base_data = Some(entry.archive.read_file_concurrent(filename)?);
                }
            }
        }
